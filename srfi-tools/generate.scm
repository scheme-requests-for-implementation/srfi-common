...
(define (find-subtag tag elements)
  (and (pair? elements)
       (or (and (pair? (car elements))
		(eqv? tag (caar elements))
		(car elements))
	   (find-subtag tag (cdr elements)))))

(define (whitespace-string? s)
  (and (string? s)
       (string-every char-whitespace? s)))

(define (strip-whitespace sxml)
  (cond ((not (pair? sxml)) sxml)
	((eqv? '@ (car sxml)) sxml)
	((symbol? (car sxml))
	 (cons (car sxml)
	       (map strip-whitespace
		    (remove whitespace-string? (cdr sxml)))))
	(else sxml)))

(define (generate-admin-page filename)
  (let* ((source-path (path-append (srfi-common-dir)
				   "admin"
				   filename))
	 (output-path (path-append (srfi-common-dir) filename))
	 (sxml (read-html-file source-path))
	 (html-elem (find-html-tag sxml))
	 (html-children (sxml-body html-elem))
	 (title-elem (or (let ((head-elem (find-subtag 'head html-children)))
			   (and head-elem
				(find-subtag 'title (sxml-body head-elem))))
			 (find-subtag 'title html-children)))
	 (title-text (if title-elem
			 (sxml-body-as-string title-elem)
			 ""))
	 (body-elem (find-subtag 'body html-children))
	 (body-children (if body-elem
			    (sxml-body (strip-whitespace body-elem))
			    '())))
    (write-html-file output-path
      `(*TOP*
	(!DOCTYPE html)
	(html (@ (lang "en"))
	  ,(admin-page-head title-text)
	  (body ,@body-children))))))
...
(define-command (generate-common)
  ...
  (generate-admin-page "about.html")
  (generate-admin-page "srfi-editors.html")
  (generate-admin-page "srfi-faq.html")
  (generate-admin-page "srfi-history.html")
  (write-srfi-home-page)
  (write-srfi-list-subscribe-page)
  (generate-admin-page "srfi-privacy.html")
  (generate-admin-page "srfi-process.html"))
